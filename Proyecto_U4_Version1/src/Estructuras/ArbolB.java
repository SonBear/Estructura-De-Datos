...
public class ArbolB<T extends Comparable<T>> implements ArbolIB<T> {
    ...
    private void recorrerArbol(Node<T> node, ArrayList<Integer> indices) {
        int x = 0;
    ...
    public boolean add(T value, int index) {
        if (root == null) {
            ...
            key.getIndiceEgresados().add(index);
            root.addKey(key);
            ...
            while (node != null) {
                ...
                for (int i = 0; i < node.numberOfKeys(); i++) {
                    if (node.getKey(i).getElemento().compareTo(value) == 0) {
                        node.getKey(i).getIndiceEgresados().add(index);
                        repitKey = true;
                        break;
    ...
    public ArrayList<Integer> buscar(T elemento) throws ItemNotFoundException {
        ...
        for (int i = 0; i < node.getKeysSize(); i++) {
            if (node.getKey(i).getElemento().compareTo(elemento) == 0) {
                return node.getKey(i).getIndiceEgresados();
            }
        }
        throw new ItemNotFoundException("Elemento no encontrado");
    }
